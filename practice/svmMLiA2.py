...
def calcEk(oS, k):
    fXk = float(np.multiply(oS.alpha, oS.labelMat).T * oS.K[:, k] + oS.b)
...
def selectJ(i, oS, Ei):
    ...
    Ej = 0
    oS.eCache[i] = [1, Ei]
    validEcachedList = np.nonzero(oS.eCache[:, 0].A)[0]
...
def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup=('lin', 0)):
    ...
    while iter < maxIter and (alphaPairChanged > 0 or entireSet):
        ...
        if entireSet:
            ...
            for i in nonBounds:
                alphaPairChanged += innerL(i, oS)
                print("non-bound, iter: %d i:%d, pair changed %d") % (iter, i , alphaPairChanged)
            iter += 1
        if entireSet:
            entireSet = False
        elif alphaPairChanged==0:
            entireSet = True
        print("iterarion number: %d" % iter)
